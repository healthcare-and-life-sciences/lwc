...
// values from LWC Properties provided in the Omniscript
    ...
    */
    ...
    handleChange(event) {
        ...
        let selectedItems = [];
        let remainingItems = this.options.slice();
        ...
        selectedOptionsList.forEach((item, i) => {
            ...
            const index = remainingItems.findIndex(x => x.value === item);
            //           alert('index ' + index);
            if (index !== undefined) remainingItems.splice(index, 1);
        });
        ...
        this.omniUpdateDataJson({"remainingSelections":remainingItems});
